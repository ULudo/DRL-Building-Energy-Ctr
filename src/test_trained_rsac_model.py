...
PARAMETER_VARS = [
    ...
    ModelVariable(name="MinimumControlEMS", value=False, unit="bool", info="Inputs are overwritten, if the EMS fails to keep the temperatures above the boundries of the standard control."),
...
OBSERVATION_VARS = [
    ...
    # ModelVariable(name="Setpoint_Act_HP_3WV", info="Actual setpoint of 3-way-valve", bounds=[0, 1]), # TODO
    ModelVariable(name="Price_elBuy", unit="EUR/kWh", info="Electricity price buy", type="data_file"), # bounds=[-.11, .55]), # TODO
    ModelVariable(name="Price_elSell", unit="EUR/kWh", info="Electricity price sell", type="data_file"), # bounds=[-.36, .3]) # TODO
]
...
INFORMATIVE_VARS = [
    ...
    ModelVariable(name="Setpoint_Act_HP_Aux", info="Actual Setpoint"),
    ModelVariable(name="HS_S_FW_HC_bM", unit="°C", info="")
]
...
rsac_algorithm.load_actor(ACTOR_PATH)

test_and_report(test_env, runs=1, epoch=0, algo=rsac_algorithm)
